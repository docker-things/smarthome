...
function show() {
    ...
    let play_img;
    if (snapstream) {
        play_img = 'res/img/screen/audio/stop.png';
    }
    else {
        play_img = 'res/img/screen/audio/play.png';
    }
    let content = "";
    ...
    for (let group of server.groups) {
        ...
        if (group.muted == true) {
            ...
            muted = true;
            mute_img = 'res/img/screen/audio/mute_icon.png';
        }
        else {
            classgroup = 'group';
            muted = false;
            mute_img = 'res/img/screen/audio/speaker_icon.png';
        }
        // Start group div
        ...
        for (let i_stream = 0; i_stream < server.streams.length; i_stream++) {
            ...
            setAudioStreamStatus(server.streams[i_stream].id, server.streams[i_stream].status)
        ...
        content += "<div class='name'>"+streamselect+"</div>";
        let clientCount = 0;
        for (let client of group.clients)
        ...
        if (clientCount > 1) {
            ...
            content += "<a href=\"javascript:setMuteGroup('" + group.id + "'," + !muted + ");\" class='mute-button'><img src='" + mute_img + "'></a>";
        ...
        for (let client of group.clients) {
            ...
            // Populate client div
            content += "<a href=\"javascript:setVolume('" + client.id + "'," + !muted + ");\" class='mute-button'><img src='" + mute_img + "'></a>";
    ...
    document.getElementById('showSnapControl').innerHTML = content;
